...
| Integração de produto | Frontend + API + PostgreSQL + OPA | `VALIDATED_INTEGRATION` (jornada principal e reconciliação) | [`Cross-repository E2E`](https://github.com/leandrosflora/intelligent-backoffice-frontend/actions/workflows/e2e.yml): gate E2E browser-based cross-repo, executado a cada PR do frontend, cobrindo `createCase → registerDocument → investigate → recommend → approve → execute` até `EXECUTED` e o caminho ambíguo até reconciliação |

## Matriz de capacidades

| Capacidade | Baseline de referência | Implementação de produto | Arquitetura-alvo | Gap principal |
|---|---|---|---|---|
| Intake e gestão de casos | `DEMONSTRATED_LOCAL`: FastAPI, idempotência, versionamento e timeline | `VALIDATED_INTEGRATION`: API .NET e telas React para criar, listar, consultar e cancelar; criação de caso coberta pelo E2E cross-repo | Case Intake API e Case Management governados | IAM corporativo, canais reais, SLOs e operação |
...
| Execução governada | `DEMONSTRATED_LOCAL`: execução mock idempotente e reconciliação | `VALIDATED_INTEGRATION` (sucesso e ambíguo): gateway mock .NET, consulta, idempotência e reconciliação; frontend cobre sucesso, falha e ambiguidade, com sucesso e ambíguo→reconciliação cobertos pelo E2E cross-repo | Serviço de execução com adapters reais | Integrações financeiras, compensação, caminho de falha no E2E e segregação operacional |
| Event backbone | `DEMONSTRATED_LOCAL`: Redpanda, outbox, inbox, retry, DLQ e replay | `VALIDATED_INTEGRATION` (dispatch): Redpanda, outbox dispatcher, workflow consumer, timer worker, DLQ e replay; dispatch de um caso real (`PENDING → PUBLISHED`) provado no [`Cross-repository E2E`](https://github.com/leandrosflora/intelligent-backoffice-frontend/actions/workflows/e2e.yml) | Kafka multi-broker com governança corporativa | DLQ replay e timer-firing ainda só cobertos por testes isolados do backend (nenhuma jornada de E2E produz esses cenários); schemas em registry, ACLs, retenção, capacidade, HA e DR |
...
| Contratos e rastreabilidade | `CONTRACT_DEFINED`: OpenAPI, AsyncAPI, schemas, policies e matriz | `VALIDATED_INTEGRATION`: `Backoffice.Contracts.Tests` compara a OpenAPI gerada pelo backend com os contratos desta arquitetura em CI; frontend implementa a jornada | Governança contínua de contratos | Depreciação e versionamento formal de mudanças de contrato |
