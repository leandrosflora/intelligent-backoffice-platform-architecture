...
def headers(role, subject="case-manager-1", subject_type="HUMAN"):
    return {"X-Subject-Id":subject,"X-Subject-Type":subject_type,"X-Roles":role,"X-Tenant-Id":"tenant-demo","X-Correlation-Id":"ci-e2e"}
...
h = headers("document-processor"); h["If-Match"] = str(case["version"])
status, case = request("POST", f"/v1/cases/{case_id}/documents", {"document_id":"doc-ci","filename":"proof.pdf"}, h)
assert status == 200, (status, case)

h = headers("operations-analyst"); h["If-Match"] = str(case["version"])
status, case = request("POST", f"/v1/cases/{case_id}/investigations", {}, h)
assert status == 200, (status, case)

h = headers("decision-agent", "decision-agent-1", "WORKLOAD"); h["If-Match"] = str(case["version"])
...
h = headers("approver", "approver-1"); h["If-Match"] = str(case["version"])
...
h = headers("execution-service", "execution-service", "WORKLOAD"); h["Idempotency-Key"] = "ci-execution-1"
